...
if (!isset($array_data["uid_place"])) exit(json_encode(array("result" => "false", "msg" => "missing_place")));

// Thêm các trường mới
$array_data["type"] = isset($array_data["type"]) ? $array_data["type"] : "";
$array_data["title"] = isset($array_data["title"]) ? $array_data["title"] : "";
...
$uid_place_user = $array_data['uid_place'];
...
if (!$array_place_result) exit(json_encode(array("result" => "false", "msg" => "missing_street_info", "debug" => $uid_str_street)));
...
/*check update*/
$uid_post_user = isset($array_data["post"]) ? $array_data["post"] : "";
if ($uid_post_user != "") {
    $array_check = $this->PostUser->read($uid_post_user);
    if ($array_check) $array_data["id"] = $array_check["id"];
}

$this->PostUser->save($array_data);

$last_uid_post_user = $uid_post_user;
if ($this->PostUser->last_action == "insert") {
    $last_uid_post_user = $this->PostUser->last_uid;
    $msg = "created";
} else {
    $msg = "updated";
}

echo json_encode(array("result" => "ok", "msg" => $msg, "data" => array("post" => $last_uid_post_user)));
