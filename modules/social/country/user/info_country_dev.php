...
if ($check["result"] != "ok") return $this->redirect(array("controller" => "country", "function" => "user", "get" => "msg=" . $check["msg"]));

$array_country_user_data = $this->request_api("country", "info_user", ["country" => $uid_country_user]);
$array_country_user = isset($array_country_user_data["data"]) ? $array_country_user_data["data"] : null;
if ($array_country_user == null) exit("Lỗi");
$form_row = "";
...
$button_approve = $Form->button(array("class" => "edit", "type" => "button", "onclick" => "approve_country('$uid_country_user')"), "Approved");
if (!$array_country_user['approved']) $form_row .= $Form->row("", $button_approve);
