...
$array_country_data = $this->request_api("country", "all", ["search" => $search, "status" => $status, "lang" => $lang]);

$array_countries = isset($array_country_data["data"]) ? $array_country_data["data"] : [];

// Duyệt tất cả dữ liệu của mảng
$str_row = "";
$i = 0;
if ($array_countries !== null) {
    ...
    foreach ($array_countries as $row) {
        ...
        $array_row["col7"] = array($row["status"] ? "Hoạt Động" : "Đang Dừng");
