...
$array_org_data = $this->request_api('org', 'all', array('search' => $search, 'area' => $area_str_code, 'lang' => $lang, 'street' => $street, 'place' => $place));
...
if ($array_org != null) {
    foreach ($array_org as $key => $row) {
        ...
        $array_row["col9"] = array($button_v);
        $str_row .= $Table->row($array_row);
    }
} else {
    $array_row = null;
    $array_row["col1"] = array("No data", array("colspan" => "9"));
    $str_row = $Table->row($array_row);
}
...
$array_header["col9"] = array("Options");
