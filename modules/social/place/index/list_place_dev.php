...
$array_user_place_data = $this->request_api("place", "list_my", ["area" => $area_str_code, "search" => $search, "lang" => $lang, "street" => $street, "place_in" => $place_in]);
$array_user_place = isset($array_user_place_data["data"]) ? $array_user_place_data["data"] : null;
$stt = 0;
if ($array_user_place != null) {
    foreach ($array_user_place as $row) {
        $stt++;
        $id = $row["id"];
        ...
        /*detail*/
        $button_detail = $Html->Form->button(array("class" => "view", "onclick" => "show_place_info('$uid_place_user')"), "View");
