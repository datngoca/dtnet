<script>
    var url_post = "<?php echo $this->get_link(array("controller" => "post", "function" => "user")) ?>";
    ...
    function change_area_list(area_code) {
        ...
        $("#uid_place").val("");
        // Reset org selection khi thay đổi area
        $("#uid_org").val("");
    ...
    function change_street_list(street_uid) {
        ...
        $("#uid_place").val("");
        // Reset org selection khi thay đổi street
        $("#uid_org").val("");
...
>>>>>>> ecb0f9b (updated from huy's computer)
    ...
    function show_post_info(view_uid_p_user) {
        ...
        var str_place_name = "";
        if ($("#place_name_" + view_uid_p_user).length) str_place_name = $("#place_name_" + view_uid_p_user).html();
        if ($("#area_str_name_" + view_uid_p_user).length) str_place_name += " - " + $("#area_str_name_" + view_uid_p_user).html();
    ...
    function show_approve_post(obj) {
        console.log("obj", obj);
        ...
        if (uid_post) show_post_info(uid_post);
    }

    function approve_post() {
        var url_approve_post = url_post;
        url_approve_post += "?action=approve&post=" + uid_post_user_selected;

        return get_data(url_approve_post, null, show_approve_post, "");
    }
</script>